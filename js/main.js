...
const DP_DEFAULT = {
  logo: null,
  title: "제목을 입력하세요",
  date: "2026.09.07 오전 10:54",
  date2: "2026.09.07 오전 10:54",
  img: null,
  body: "[매체명=기자명] 여기에 기사 본문을 입력하세요.\n\n문단을 나누고 싶으면 줄바꿈을 두 번 하면 됩니다. 굵게 하고 싶은 부분은 드래그한 뒤 'B 굵게' 버튼을 누르세요.",
  caption: "<사진제공=OOO>",
  related: [
    { img: null, title: "관련 기사 제목 예시 1", date: "2026.09.06" },
    { img: null, title: "관련 기사 제목 예시 2", date: "2026.09.06" },
...
function dcLoad() {
  try {
    const raw = sessionStorage.getItem(DC_STORAGE_KEY);
    if (raw) return Object.assign(dcCloneDefault(), JSON.parse(raw));
  } catch (e) {}
  return dcCloneDefault();
}
let dcState = dcLoad();
...
function dcRenderAll() {
  document.getElementById("dcOutGalleryText").textContent = dcState.gallery;
  document.getElementById("dcOutTitle").textContent = dcState.title;
  document.getElementById("dcOutNick").textContent = dcState.nick;
  document.getElementById("dcOutNickIp").textContent = dcState.nickIp ? `(${dcState.nickIp})` : "";
  document.getElementById("dcOutDate").textContent = dcState.date;
  document.getElementById("dcOutViews").textContent = dcState.views;
  document.getElementById("dcOutLikes").textContent = dcState.likes;
  document.getElementById("dcOutLikes2").textContent = dcState.likes;
  document.getElementById("dcOutComments").textContent = dcState.comments;

  const imgEl = document.getElementById("dcOutImg");
  if (dcState.img) { imgEl.src = dcState.img; imgEl.classList.remove("hidden"); }
  else { imgEl.src = ""; imgEl.classList.add("hidden"); }

  const bodyEl = document.getElementById("dcOutBody");
  bodyEl.className = "dc-body " + ("font-" + (dcState.font || "pretendard"));
  bodyEl.style.fontSize = (dcState.fontSize || "15") + "px";
  bodyEl.style.color = "#1a1a1a";
  bodyEl.innerHTML = (dcState.body || "").split(/\n\s*\n/).map(p =>
    `<p>${dcFormatBody(p).replace(/\n/g,"<br/>")}</p>`
  ).join("");

  const list = dcState.comments_list || [];
  document.getElementById("dcOutCommentCount").textContent = `[${list.length}]`;
  document.getElementById("dcOutCommentList").innerHTML = list.map(c => `
    <div class="dc-comment-item">
      <span class="dc-comment-nick">${dcEscHtml(c.nick)}</span>${c.ip ? ` <span class="dc-ip">(${dcEscHtml(c.ip)})</span>` : ""}
      <div class="dc-comment-text">${dcEscHtml(c.text)}</div>
      <div class="dc-comment-date">${dcEscHtml(c.date)}</div>
    </div>`).join("");

  dcRenderCommentEditor();
  dcSave();
}

function dcRenderCommentEditor() {
  const wrap = document.getElementById("dcCommentEditList");
  const list = dcState.comments_list || [];
  wrap.innerHTML = list.map((c, i) => `
    <div class="comment-edit-card" data-dc-comment-card="${i}">
      <div class="row">
        <div class="field"><label>닉네임</label><input type="text" data-dc-c="${i}" data-dc-k="nick" value="${dcEscHtml(c.nick)}" /></div>
        <div class="field"><label>아이피 (선택)</label><input type="text" data-dc-c="${i}" data-dc-k="ip" value="${dcEscHtml(c.ip)}" /></div>
      </div>
      <div class="field"><label>날짜/시간</label><input type="text" data-dc-c="${i}" data-dc-k="date" value="${dcEscHtml(c.date)}" /></div>
      <div class="field"><label>내용</label><textarea rows="2" data-dc-c="${i}" data-dc-k="text">${dcEscHtml(c.text)}</textarea></div>
      <button type="button" class="comment-edit-remove" data-dc-remove="${i}">이 댓글 삭제</button>
    </div>`).join("");

  wrap.querySelectorAll("[data-dc-c]").forEach(el => {
    const handler = (e) => {
      const i = +el.dataset.dcC, k = el.dataset.dcK;
      dcState.comments_list[i][k] = e.target.value;
      document.getElementById("dcOutCommentCount").textContent = `[${dcState.comments_list.length}]`;
      const list2 = dcState.comments_list;
      document.getElementById("dcOutCommentList").innerHTML = list2.map(c => `
        <div class="dc-comment-item">
          <span class="dc-comment-nick">${dcEscHtml(c.nick)}</span>${c.ip ? ` <span class="dc-ip">(${dcEscHtml(c.ip)})</span>` : ""}
          <div class="dc-comment-text">${dcEscHtml(c.text)}</div>
          <div class="dc-comment-date">${dcEscHtml(c.date)}</div>
  ...
  wrap.querySelectorAll("[data-dc-remove]").forEach(el => {
    el.addEventListener("click", () => {
      dcState.comments_list.splice(+el.dataset.dcRemove, 1);
      dcRenderAll();
    });
  });
}

document.getElementById("dcCommentAddBtn").addEventListener("click", () => {
  dcState.comments_list.push({ nick: "ㅇㅇ", ip: "", date: "", text: "" });
  dcRenderAll();
});

document.getElementById("dcBoldBtn").addEventListener("click", () => {
  const ta = document.getElementById("dcBody");
  const start = ta.selectionStart, end = ta.selectionEnd;
  if (start === end) return;
  ta.value = ta.value.slice(0, start) + "**" + ta.value.slice(start, end) + "**" + ta.value.slice(end);
  dcState.body = ta.value;
  dcRenderAll();
  ta.focus();
});
document.getElementById("dcColorBtn").addEventListener("click", () => {
  const ta = document.getElementById("dcBody");
  const start = ta.selectionStart, end = ta.selectionEnd;
  if (start === end) return;
  const color = document.getElementById("dcSelColor").value;
  const open = `{{color:${color}}}`, close = "{{/color}}";
  ta.value = ta.value.slice(0, start) + open + ta.value.slice(start, end) + close + ta.value.slice(end);
  dcState.body = ta.value;
  dcRenderAll();
  ta.focus();
});
...
function dcFillForm() {
  document.getElementById("dcGallery").value = dcState.gallery;
  document.getElementById("dcTitle").value = dcState.title;
  document.getElementById("dcNick").value = dcState.nick;
  document.getElementById("dcNickIp").value = dcState.nickIp;
  document.getElementById("dcDate").value = dcState.date;
  document.getElementById("dcViews").value = dcState.views;
  document.getElementById("dcLikes").value = dcState.likes;
  document.getElementById("dcComments").value = dcState.comments;
  document.getElementById("dcBody").value = dcState.body;
  document.getElementById("dcFont").value = dcState.font;
  document.getElementById("dcFontSize").value = dcState.fontSize;
}

function dcBindInput(id, key) {
  const el = document.getElementById(id);
  const handler = (e) => { dcState[key] = e.target.value; dcRenderAll(); };
  el.addEventListener("input", handler);
  el.addEventListener("change", handler);
}
["gallery","title","nick","nickIp","date","views","likes","comments","body","font","fontSize"].forEach(key => {
  const idMap = { gallery:"dcGallery", title:"dcTitle", nick:"dcNick", nickIp:"dcNickIp", date:"dcDate", views:"dcViews", likes:"dcLikes", comments:"dcComments", body:"dcBody", font:"dcFont", fontSize:"dcFontSize" };
  dcBindInput(idMap[key], key);
});

document.getElementById("dcImgInput").addEventListener("change", (e) => {
  const file = e.target.files[0];
  if (!file) return;
  const reader = new FileReader();
  reader.onload = (ev) => { dcState.img = ev.target.result; dcRenderAll(); };
  reader.readAsDataURL(file);
});
document.getElementById("dcRemoveImgBtn").addEventListener("click", () => {
  dcState.img = null;
  document.getElementById("dcImgInput").value = "";
  dcRenderAll();
});

document.getElementById("dcResetBtn").addEventListener("click", () => {
  if (!confirm("디시인사이드 입력 내용을 기본값으로 되돌릴까요?")) return;
  dcState = dcCloneDefault();
  try { sessionStorage.removeItem(DC_STORAGE_KEY); } catch (e) {}
  document.getElementById("dcImgInput").value = "";
  dcFillForm();
  dcRenderAll();
});

document.getElementById("dcSavePngBtn").addEventListener("click", () => {
  const target = document.getElementById("previewDc");
  const btn = document.getElementById("dcSavePngBtn");
  const originalText = btn.textContent;
  if (typeof html2canvas === "undefined") {
    alert("이미지 생성 기능을 불러오지 못했습니다. 인터넷 연결을 확인한 뒤 새로고침해서 다시 시도해주세요.");
    return;
  ...
  ready.then(() => html2canvas(target, {
    ...
    canvas.toBlob((blob) => {
      ...
      const url = URL.createObjectURL(blob);
      const link = document.createElement("a");
      link.download = "dcinside_post.png";
      link.href = url;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
...
const WINDOWS_DEFAULT_WALLPAPER = "data:image/webp;base64,";
const WINDOWS_KEY = "virtual_windows_desktop_v1";
const WINDOWS_DEFAULT = {wallpaper:WINDOWS_DEFAULT_WALLPAPER,apps:[]};
let windowsState;
try{windowsState=JSON.parse(localStorage.getItem(WINDOWS_KEY)||"null")||JSON.parse(JSON.stringify(WINDOWS_DEFAULT));}catch(e){windowsState=JSON.parse(JSON.stringify(WINDOWS_DEFAULT));}
function windowsSave(){try{localStorage.setItem(WINDOWS_KEY,JSON.stringify(windowsState));}catch(e){}}
function windowsEsc(s){return String(s??"").replace(/[&<>"']/g,m=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[m]));}
function windowsRender(){
 const c=document.getElementById("windowsCanvas"); if(!c)return;
 c.style.backgroundImage='url("'+(windowsState.wallpaper||WINDOWS_DEFAULT_WALLPAPER)+'")'; c.innerHTML="";
 windowsState.apps.forEach((a,i)=>{
  const e=document.createElement("div"); e.className="windows-icon";
  e.style.left=(a.x??5)+"%"; e.style.top=(a.y??5)+"%"; e.style.width=(a.width??8)+"%";
  e.innerHTML='<img src="'+a.src+'" alt="" style="width:'+Number(a.size||64)+'px;height:'+Number(a.size||64)+'px"><span class="windows-icon-name">'+windowsEsc(a.name||"앱")+"</span>";
  c.appendChild(e);
 });
}
function windowsEditors(){
 const box=document.getElementById("windowsAppList"); if(!box)return; box.innerHTML="";
 windowsState.apps.forEach((a,i)=>{
  const card=document.createElement("div"); card.className="windows-app-card";
  card.innerHTML='<div class="windows-app-top"><span class="windows-app-title">앱 '+(i+1)+'</span><button type="button" class="windows-remove" data-i="'+i+'">삭제</button></div>'+
  '<div class="field"><label>앱 아이콘</label><input class="windows-file" type="file" accept="image/*" data-i="'+i+'" data-kind="icon"></div>'+
  '<div class="field"><label>앱 이름</label><input type="text" value="'+windowsEsc(a.name||"")+'" data-i="'+i+'" data-kind="name" placeholder="예: Chrome"></div>'+
  '<div class="row"><div class="field"><label>위치 X (%)</label><input type="number" min="0" max="93" step="0.5" value="'+(a.x??5)+'" data-i="'+i+'" data-kind="x"></div>'+
  '<div class="field"><label>위치 Y (%)</label><input type="number" min="0" max="88" step="0.5" value="'+(a.y??5)+'" data-i="'+i+'" data-kind="y"></div></div>'+
  '<div class="field"><label>아이콘 크기 (px)</label><input type="number" min="24" max="128" step="1" value="'+(a.size??64)+'" data-i="'+i+'" data-kind="size"></div>';
  box.appendChild(card);
 });
 box.querySelectorAll("[data-kind]").forEach(inp=>{
  inp.addEventListener("input",e=>{const i=+e.target.dataset.i,k=e.target.dataset.kind;if(k==="icon")return;windowsState.apps[i][k]=k==="name"?e.target.value:Number(e.target.value);windowsSave();windowsRender();});
  inp.addEventListener("change",e=>{const i=+e.target.dataset.i,k=e.target.dataset.kind;if(k!=="icon")return;const f=e.target.files&&e.target.files[0];if(!f)return;const r=new FileReader();r.onload=()=>{windowsState.apps[i].src=r.result;windowsSave();windowsRender();};r.readAsDataURL(f);});
 });
 box.querySelectorAll(".windows-remove").forEach(b=>b.addEventListener("click",()=>{windowsState.apps.splice(+b.dataset.i,1);windowsSave();windowsEditors();windowsRender();}));
}
document.getElementById("windowsWallpaper")?.addEventListener("change",e=>{const f=e.target.files&&e.target.files[0];if(!f)return;const r=new FileReader();r.onload=()=>{windowsState.wallpaper=r.result;windowsSave();windowsRender();};r.readAsDataURL(f);});
document.getElementById("windowsAddApp")?.addEventListener("click",()=>{
 const n=windowsState.apps.length;
 windowsState.apps.push({name:"앱",src:"data:image/svg+xml;charset=UTF-8,"+encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64"><rect width="64" height="64" rx="10" fill="#ddd"/><text x="32" y="38" text-anchor="middle" font-size="12" fill="#666">APP</text></svg>'),x:5+(n%4)*9,y:5+Math.floor(n/4)*13,size:64,width:8});
 windowsSave();windowsEditors();windowsRender();
});
document.getElementById("windowsReset")?.addEventListener("click",()=>{windowsState=JSON.parse(JSON.stringify(WINDOWS_DEFAULT));windowsSave();windowsEditors();windowsRender();});
windowsEditors();windowsRender();
...
(function(){const D={username:"_starhwa_",name:"박성화",posts:"4",followers:"5,384",following:"26",bio:"개 산책시키는 사람",link:"youtu.be/marspark03hsdqwjhea",follow:"팔로우",message:"메시지",highlight:"✈️"};const DEF=["", "", "", ""];let imgs;try{imgs=JSON.parse(sessionStorage.getItem('igp_posts_v1'))||DEF}catch(e){imgs=DEF};const map={username:['igpUsername','igpOutUsername'],name:['igpName','igpOutName'],posts:['igpPosts','igpOutPosts'],followers:['igpFollowers','igpOutFollowers'],following:['igpFollowing','igpOutFollowing'],bio:['igpBio','igpOutBio'],link:['igpLink','igpOutLink'],follow:['igpFollowText','igpOutFollow'],message:['igpMessageText','igpOutMessage'],highlight:['igpHighlightTitle','igpOutHighlightTitle']};let saved={};try{saved=JSON.parse(sessionStorage.getItem('igp_state_v1'))||{}}catch(e){};Object.entries(map).forEach(([k,v])=>{const e=document.getElementById(v[0]),o=document.getElementById(v[1]);if(!e||!o)return;e.value=saved[k]??D[k];o.textContent=e.value;e.addEventListener('input',()=>{o.textContent=e.value;save()})});function save(){const o={};Object.entries(map).forEach(([k,v])=>o[k]=document.getElementById(v[0]).value);try{sessionStorage.setItem('igp_state_v1',JSON.stringify(o));sessionStorage.setItem('igp_posts_v1',JSON.stringify(imgs))}catch(e){}}function editors(){const box=document.getElementById('igpPostEditors');box.innerHTML='';imgs.forEach((s,i)=>{const d=document.createElement('div');d.style='display:flex;gap:7px;margin-bottom:7px;align-items:center';d.innerHTML=`<span style="width:22px;font-size:12px">${i+1}</span><input type="file" accept="image/*" style="flex:1"><button type="button" class="btn btn-ghost" style="width:auto;padding:7px 10px">삭제</button>`;d.querySelector('input').addEventListener('change',e=>{const f=e.target.files?.[0];if(!f)return;const r=new FileReader();r.onload=()=>{imgs[i]=r.result;save();render()};r.readAsDataURL(f)});d.querySelector('button').addEventListener('click',()=>{imgs.splice(i,1);save();render()});box.appendChild(d)})}function render(){const g=document.getElementById('igpOutGrid');g.innerHTML='';imgs.forEach(s=>{if(!s)return;const im=document.createElement('img');im.className='igp-post';im.src=s;g.appendChild(im)});editors()}function syncHighlightVisibility(){const cb=document.getElementById('igpShowHighlight'),el=document.querySelector('.igp-highlight');if(!cb||!el)return;el.style.display=cb.checked?'':'none'}document.getElementById('igpShowHighlight')?.addEventListener('change',()=>{syncHighlightVisibility();try{sessionStorage.setItem('igp_show_highlight_v1',document.getElementById('igpShowHighlight').checked?'1':'0')}catch(e){}});try{const sh=sessionStorage.getItem('igp_show_highlight_v1');if(sh!==null)document.getElementById('igpShowHighlight').checked=sh==='1'}catch(e){}syncHighlightVisibility();
document.getElementById('igpAvatar')?.addEventListener('change',e=>{const f=e.target.files?.[0];if(!f)return;const r=new FileReader();r.onload=()=>{document.getElementById('igpOutAvatar').src=r.result;try{sessionStorage.setItem('igp_avatar_v1',r.result)}catch(e){}};r.readAsDataURL(f)});document.getElementById('igpHighlight')?.addEventListener('change',e=>{const f=e.target.files?.[0];if(!f)return;const r=new FileReader();r.onload=()=>{document.getElementById('igpOutHighlight').src=r.result;try{sessionStorage.setItem('igp_highlight_v1',r.result)}catch(e){}};r.readAsDataURL(f)});try{const a=sessionStorage.getItem('igp_avatar_v1'),h=sessionStorage.getItem('igp_highlight_v1');if(a)document.getElementById('igpOutAvatar').src=a;if(h)document.getElementById('igpOutHighlight').src=h}catch(e){}document.getElementById('igpAddPost')?.addEventListener('click',()=>{if(imgs.length<6){imgs.push('');save();render()}});document.getElementById('igpReset')?.addEventListener('click',()=>{if(!confirm('인스타그램 프로필을 기본값으로 복원할까요?'))return;Object.entries(map).forEach(([k,v])=>{document.getElementById(v[0]).value=D[k];document.getElementById(v[1]).textContent=D[k]});imgs=DEF.slice();try{sessionStorage.removeItem('igp_state_v1');sessionStorage.removeItem('igp_avatar_v1');sessionStorage.removeItem('igp_highlight_v1')}catch(e){}document.getElementById('igpOutAvatar').src="";document.getElementById('igpOutHighlight').src="";save();render()});document.getElementById('igpSavePng')?.addEventListener('click',()=>{const t=document.getElementById('instagramProfilePreview'),b=document.getElementById('igpSavePng');if(!t||typeof html2canvas==='undefined'){alert('이미지 생성 기능을 불러오지 못했습니다.');return}b.disabled=true;const old=b.textContent;b.textContent='이미지 생성 중...';html2canvas(t,{backgroundColor:'#fff',scale:3,useCORS:true,allowTaint:true,logging:false}).then(c=>c.toBlob(blob=>{const u=URL.createObjectURL(blob),a=document.createElement('a');a.href=u;a.download='instagram_profile.png';a.click();setTimeout(()=>URL.revokeObjectURL(u),1500);b.textContent=old;b.disabled=false},'image/png')).catch(()=>{alert('PNG 저장에 실패했습니다.');b.textContent=old;b.disabled=false})});render()})();
...
(function(){
  ...
  // 모든 body 변화를 감시하면 다른 생성기 렌더링까지 매번 전체 DOM을 스캔해 로딩이 길어집니다.
  // 현재 페이지의 프리뷰는 초기 렌더링 시점에 이미 존재하므로 최초 1회만 스캔합니다.
  scan();
})();
